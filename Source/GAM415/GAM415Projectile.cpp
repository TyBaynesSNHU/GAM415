...
AGAM415Projectile::AGAM415Projectile() 
{
	...
	ballMesh = CreateEditorOnlyDefaultSubobject<UStaticMeshComponent>("Ball Mesh");
...
void AGAM415Projectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (OtherActor != nullptr)
	{
		...
		//Assign the random values to the parameters of a 4 vector with opacity always at full
		FVector4 randColor = FVector4(ranNumX, ranNumY, ranNumZ, 1.f);

		//Places decal in world based on these metrics: world location, baseMat/material, size is random between 20 and 40 units, grabs the hit location, rotates the normals of the decal to face the camera, infinite lifespan
		auto Decal = UGameplayStatics::SpawnDecalAtLocation(GetWorld(), baseMat, FVector(UKismetMathLibrary::RandomFloatInRange(20.f, 40.f)), Hit.Location, Hit.Normal.Rotation(), 0.f);
		auto MatInstance = Decal->CreateDynamicMaterialInstance();
		
		//Sets the parameters in the material
